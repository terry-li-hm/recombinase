...
### Added
- `greyscale_photos` config option: converts inserted photos to greyscale
  (luminance) before embedding. Requires `pip install recombinase[greyscale]`
  (Pillow). Source photos can be colour — conversion happens at generate time.

## [0.2.2] - 2026-04-12

### Added
- Multi-document YAML support: files with `---` separators produce one record
  per document. Allows packing all records into a single file for easy
  copy-paste workflows.

## [0.2.1] - 2026-04-12

### Added
- `sort_by` config option: sort output slides by a field in each record YAML
  instead of filename order. Numeric values sort numerically, strings
  lexicographically, missing values sort last, equal values preserve filename
  order (stable sort).
